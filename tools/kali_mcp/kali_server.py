...
COMMAND_TIMEOUT = 1200  # 20 minutes default timeout
...
def execute_command(command: str) -> Dict[str, Any]:
    ...
    """
    # Intercept msfconsole commands to run via Docker
    logger.info(f"Original command: {command}")
    
    # Handle 'msfconsole' or 'which msfconsole' or '/usr/bin/msfconsole'
    # Force ANY msfconsole execution to check Docker presence or run via Docker
    
    # Special case: 'which msfconsole' -> return True (mock) to satisfy checks?
    # No, let's just log it.
    
    if "msfconsole" in command and not "docker" in command:
        if command.strip() == "which msfconsole":
            # If checking for existence, we can lie and say it exists via wrapper logic,
            # OR we can let it fail but ensure actual execution works.
            # User agent logic usually checks 'which' then runs.
            # Let's intercept execution, but let 'which' fail naturally unless we want to mock it.
            pass 
            
        # Native msfconsole - Docker wrapper removed (native installation used)
        # elif command.strip().startswith("msfconsole"):
        #     ... Docker code removed ...
...
def nmap():
    ...
    try:
        ...
        additional_args = params.get("additional_args", "-T4 -Pn -n")
...
def metasploit():
    ...
    try:
        ...
        # Use native msfconsole (Docker wrapper removed)
